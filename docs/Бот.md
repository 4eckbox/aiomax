...
- `secret: str | None` - секрет webhook, который MAX будет отправлять в заголовке `X-Max-Bot-Api-Secret`. Допустимая длина — от 5 до 256 символов; разрешены латинские буквы, цифры и дефис. Некорректное значение вызывает `ValueError` до API-запроса.
